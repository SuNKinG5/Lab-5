...
class CategoryTest {
	...
	ShippingVehicle pack = new ShippingVehicle(); 

	@ParameterizedTest
	@MethodSource("TC")
	void test(int small, int mid, int large, List<Integer> expected) {
		
		assertEquals(expected, pack.calculate(small, mid, large));
	}
	...
	static Stream<Arguments> TC() {
		return Stream.of(
			...
			Arguments.of(0, 2, 99, List.of(-1)),
			Arguments.of(3, 199, 0, List.of(-1)),
			Arguments.of(1, 50, 90, List.of(-1)),
